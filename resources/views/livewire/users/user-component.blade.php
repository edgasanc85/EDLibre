<div>
    <div class="container-fluid px-4 py-4">
        <div class="d-flex justify-content-between align-items-center mb-4">
            <h1 class="h3 mb-0 text-dark fw-bold">Gestión de Usuarios</h1>
                ...
                <div class="modal-dialog modal-lg modal-dialog-centered">
                    <div class="modal-content border-0 shadow-lg rounded-3">
                        <div class="modal-header bg-light border-0 py-3 px-4 border-bottom">
                            <h5 class="modal-title text-dark fw-bold"><i class="bi bi-person-plus me-2" style="color: var(--ea-primary);"></i>{{ $isEditMode ? 'Editar Usuario' : 'Crear Nuevo Usuario' }}</h5>
                        ...
                        <form wire:submit.prevent="{{ $isEditMode ? 'update' : 'store' }}">
                            <div class="modal-body p-4">
                                <div class="row g-3">
                                    ...
                                    <div class="col-md-6">
                                        <label class="form-label text-muted fw-bold">Contraseña <small class="fw-normal">{{ $isEditMode ? '(Dejar en blanco para no cambiar)' : '' }}</small></label>
                                        <input type="password" wire:model="password" class="form-control @error('password') is-invalid @enderror">
                                        @error('password') <span class="text-danger small">{{ $message }}</span> @enderror
                                    </div>
                                    <div class="col-md-6 d-flex align-items-center mt-md-5">
                                        <div class="form-check form-switch">
                                            <input class="form-check-input" type="checkbox" role="switch" id="isAdminSwitch" wire:model="is_admin">
                                            <label class="form-check-label fw-bold text-dark" for="isAdminSwitch">¿Es Administrador?</label>
                                        </div>
                                    </div>
                                </div>
                            </div>
        ...
        <div class="card shadow-sm">
            <div class="card-header bg-transparent d-flex justify-content-between align-items-center py-3 border-bottom">
                ...
                <div class="position-relative" style="width: 250px;">
                    <input type="text" wire:model.live.debounce.300ms="search" class="form-control form-control-sm ps-4" placeholder="Buscar usuarios...">
                    ...
                    <table class="table table-hover mb-0">
                        <thead>
                            <tr>
                                ...
                                <th class="py-3 px-4 text-muted fw-bold">Email</th>
                                <th class="py-3 px-4 text-muted fw-bold">Rol</th>
                                ...
                                <tr>
                                    <td class="py-3 px-4 align-middle">
                                        <div class="d-flex align-items-center gap-3">
                                            ...
                                            <span class="text-dark fw-semibold">{{ $record->name }}</span>
                                        </div>
                                    </td>
                                    <td class="py-3 px-4 align-middle">
                                        <div class="small text-muted">{{ $record->tipo_documento }}</div>
                                        <div class="text-dark">{{ $record->numero_documento }}</div>
                                    </td>
                                    <td class="py-3 px-4 align-middle text-dark">{{ $record->email }}</td>
                                    <td class="py-3 px-4 align-middle">
                                        @if($record->is_admin)
                                            <span class="badge bg-primary bg-opacity-10 text-primary px-2 py-1 rounded-pill">Admin</span>
                                        @else
                                            <span class="badge bg-secondary bg-opacity-10 text-secondary px-2 py-1 rounded-pill">Usuario</span>
                                        @endif
                                    </td>
                                    <td class="py-3 px-4 align-middle text-end">
                                        ...
                                        <button onclick="if(confirm('¿Está seguro de eliminar este usuario?')) { @this.delete({{ $record->id }}) }" class="btn btn-sm btn-icon btn-outline-danger border-0" title="Eliminar">
